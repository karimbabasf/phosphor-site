...
- the security section is one panel of two cells on a hairline, Runs on its own (the agent, your rules) and Needs you (you, the Secure Enclave), the wall between them a shade stronger than the panel's rules and the second cell a shade lighter; one road runs through the four stops and crosses the wall once, laid as three stretches between the tiles and a tail after the last, each stopping 8px short of the tile at either end, so the line docks at a stop and never runs through one. The proposal is the app's card in one line (Long ETH, $150 in Geist Mono, its state at the right) and lands under the words of the stop it has reached: it appears with the agent as Drafted, folds to a dot that rides the stretch to the rules and opens there as Checked, then to You as Waiting for you, over the Approve button, and waits until the button is pressed; then the button reads Touch ID, the card travels to the enclave and opens as Signed, the dot runs down the tail, and the flow resets. The stretch behind the dot lights in the colour of whoever drove it, so the agent's violet stretch ends at the rules. Approve is the one real control on the page, hollow until it is the visitor's turn. Under each pair of stops, on a rule, the list of what the agent can (left) and cannot (right) do. Under 1200px the cells stack and the road runs top to bottom: from 720px each list sits beside its stops, and on a phone the four stops come first and both lists after, so the road meets nothing but the wall; with reduced motion the card simply waits at You
