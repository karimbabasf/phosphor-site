...
- the security section is a flow of four nodes (the agent, your rules, you, the Secure Enclave) with a proposal that travels them: it appears with the agent, stops at the rules and gets its green dot, and waits at You until the Approve button is pressed; then the button reads Touch ID, the proposal moves to the enclave as signed, leaves, and the flow resets. Approve is the one real control on the page. On a phone the flow runs top to bottom. With reduced motion the proposal simply waits at You
